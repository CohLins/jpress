...
public class UserFavoriteServiceProvider extends JbootServiceBase<UserFavorite> implements UserFavoriteService {
    ...
    @Override
    public Page<UserFavorite> paginateByUserIdAndType(int page, int pagesize, Long userId, String type,String title) {
        return paginateByColumns(page,pagesize, Columns.create("user_id",userId).eq("type",type).likeAppendPercent("title",title),"id desc");
    }
    ...
    @Override
    public boolean isGoodFav(Long userId, Long id){
        return isFav(userId,"good",id);
    }
    @Override
    public boolean isFollowFav(Long userId, Long id){
        return isFav(userId,"follow",id);
    }
}
