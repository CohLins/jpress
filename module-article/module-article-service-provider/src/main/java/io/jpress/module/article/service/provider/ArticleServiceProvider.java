...
public class ArticleServiceProvider extends JbootServiceBase<Article> implements ArticleService {
    ...
    @Override
    @Cacheable(name = "articles")
    public Page<Article> paginateInNormal(int page, int pagesize, String orderBy,String userId) {
        orderBy = StrUtil.obtainDefaultIfBlank(orderBy, DEFAULT_ORDER_BY);
        Columns columns = new Columns();
    ...
    @Override
    @Cacheable(name = "articles")
    public Page<Article> paginateInNormal(int page, int pagesize, String orderBy,String userId,String keyword) {
        orderBy = StrUtil.obtainDefaultIfBlank(orderBy, DEFAULT_ORDER_BY);
        Columns columns = new Columns();
        columns.eq("user_id",userId);
        columns.add("status", Article.STATUS_NORMAL);
    ...
    @Override
    @Cacheable(name = "articles")
    public List<Article> paginateInNormal(String userId){
        Columns columns = new Columns();
        columns.eq("user_id",userId);
        columns.add("status", Article.STATUS_NORMAL);
        List<Article> listByColumns = DAO.findListByColumns(columns);
    ...
    @Override
    @Cacheable(name = "articles")
    public Page<Article> searchByTypeKeyword(int page, int pagesize, long categoryId, String orderBy,String keyword) {

        Columns columns = new Columns();
        columns.add("m.category_id", categoryId);
        columns.add("article.status", Article.STATUS_NORMAL);
        columns.likeAppendPercent("article.title",keyword);
    ...
    @Override
    @Cacheable(name = "articles")
    public Page<Article> searchByTypeUserId(int page, int pagesize, long categoryId, String orderBy,Long userId) {
    ...
    @Override
    @Cacheable(name = "articles")
    public Page<Article> searchByUserId(Long queryString, int pageNum, int pageSize) {
        Columns columns = Columns.create("status", Article.STATUS_NORMAL)
                .eq("user_id", queryString);
        return joinUserInfo(paginateByColumns(pageNum, pageSize, columns, "order_number desc,id desc"));
    }
